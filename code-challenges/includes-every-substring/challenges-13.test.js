...
const firstLetters = array => {
  return array.map(value => value.charAt(0));
};
...
const standardizePhoneNumbers = array  => {
...
const allHappy = array => {
  return array.every(value => value.includes(':)'));
};
...
const findAnything = (array, target) => {
  return array.filter(value => value.includes(target));
};
...
const findEvery = (array, target) => {
  return array.every(value => value.includes(target));
};
...
const unenrollBrook = array  => {
  return array .map(course => course.filter(value => !value.includes('Brook')));
};
