...
const filterStringsWithVowels = array => {
  return array.filter(word => /[aeiou]/.test(word));
};
...
const notInFirstArray = (forbiddenValues, array) => {

  return array.filter(num => !forbiddenValues.includes(num));
};
...
const getStatName = (array, minBaseStat) => {
  return getBaseStatGreaterThan(array, minBaseStat).map(s => s.stat.name);
};
...
const getCharactersWithoutChildren = array => {
  return array.filter(c => !c.children);
};
